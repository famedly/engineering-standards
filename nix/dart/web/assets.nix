  ...
  imports = [
    (import ../project-options.nix { inherit lib flake-parts-lib; } {
      options.web.livekitE2eeWorker.enable = lib.mkEnableOption ''
        ...
        `dart_webrtc` starts it by name from the site root, and without it a
        call joins but carries no media. Compiled from the `livekit_client` the
        project already resolved, so it needs no pin of its own.

        Leaves `web/e2ee.worker.dart.js`, which the project's `.gitignore` has
        to cover
      '';
    })
  ];
