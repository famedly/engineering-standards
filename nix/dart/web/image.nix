        ...
        options.web.image = {
          ...
          cacheControl = lib.mkOption {
            description = ''
              ...
              Off, because its heuristic caches every file for a day, including
              the entry document — a browser would sit on yesterday's build for
              a day after a deployment. Validators are sent either way.
          ...
          headers = lib.mkOption {
            description = ''
              ...
              No `Content-Security-Policy`: a policy that fits one application
              forbids another one's inline bootstrap, so it belongs to the
              project.
            '';
          ...
          sentHeaders = lib.mkOption {
            description = ''
              What the server is configured with: `headers`, plus the isolation
              pair when `crossOriginIsolation` asks for it. Derived, so the
              image and the test that fetches from it cannot disagree.
            '';
          ...
          crossOriginIsolation = lib.mkOption {
            description = ''
              ...
              Off, because `Cross-Origin-Embedder-Policy` blocks every cross-
              origin resource that does not opt in — fonts, images, frames.

              Turn it on for a Flutter web build with the threaded renderer,
              which needs a `SharedArrayBuffer`.
            '';
          ...
          contentTypes = lib.mkOption {
            description = ''
              ...
              A browser will not execute a module script that is not typed as
              JavaScript, nor instantiate a WebAssembly module that is not typed
              as such, so serving these wrong takes the whole application down
              where no build step is watching.
          ...
          runners = {
            ...
            arm64 = lib.mkOption {
              description = ''
                Runner that assembles the arm64 image. The standard one: only
                the server in this image is architecture-specific.
              '';
              type = lib.types.str;
