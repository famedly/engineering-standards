...
rustPlatform.buildRustPackage {
  ...
  # Darwin has no sandbox to give the build a constant directory, so the random
  # path reaches the debug info, the panic locations and LLVM's module hashes.
  ...
  # A pure `cdylib`/`staticlib`: `cargo test` would only relink it.
  doCheck = false;
